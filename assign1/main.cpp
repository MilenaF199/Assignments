...
void parse_csv(std::string filename, std::vector<Course> courses) {
  ...
  while (std::getline(file,line)){
    ...
    //create a vector to contain lines
    std:: vector<std::string> lines = split(line, ',');

    if (lines.size()< 3){
      std::cerr<< "There's no 3 elements in the line: " << line << std::endl;
      continue;
    }
    ...
    course.number_of_units = std::stoi(lines[1]);
    course.quarter = lines[2];

    //put course to courses
